...
class Constant {
  ...
  static Future<void> redirectMap({
    ...
    if (Constant.mapType == "google") {
      final googleMapsUrl =
          'comgooglemaps://?daddr=$latitude,$longLatitude&directionsmode=driving';
      final googleMapsFallbackUrl =
          'https://www.google.com/maps/dir/?api=1&destination=$latitude,$longLatitude&travelmode=driving';

      if (await canLaunch(googleMapsUrl)) {
        await launch(googleMapsUrl);
      } else {
        // If Google Maps is not installed, open the fallback web URL
        if (await canLaunch(googleMapsFallbackUrl)) {
          await launch(googleMapsFallbackUrl);
        } else {
  ...
  Future<PlacesDetailsResponse?> handlePressButton(BuildContext context) async {
    ...
    final p = await PlacesAutocomplete.show(
        ...
        mode: Mode.overlay,
        language: 'en',
        components: [Component(Component.country, 'IQ')], // Restrict to US
